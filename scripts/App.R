...
source('utils.R')
"
This shinny app includes an interactive map to visualize schools, relationships,
buffers, convex hulls, and market results. Additionally, summary statistics tables 
are included to provide information about school markets composition. 
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem("Mapa de exploración", tabName = "tab_map", 
               icon = icon("globe", lib = "glyphicon")),
      menuItem("Redes", tabName = "tab_networks", 
  ...
  dashboardBody(
    ...
    tabItems(
              ...
              fluidRow(
                ...
                column(width = 3,
                       ...
                       box(width = NULL,collapsible=T, collapsed=F, 
                           status = "info",solidHeader=T, title="Opciones",
                           ...
                           numericInput("cz_id", "Zona de desplazamiento", 
                                        value = 555),
                           # uiOutput("cz_id"),
                           # Select algorithm for create communities
                           ...
                           footer = "Se creará la carpeta \"results\" con todas 
      ...
      tabItem(tabName = "tab_networks",
              p("En esta pestaña se adicionará visualización par análisis 
                detallado de los mercados generados con:"),
              p(a("sigmajs", href="https://sigmajs.john-coene.com/"),
                "y/o", a("igraph", href="https://igraph.org/r/doc/"))
      )
    )
  ))
...
server <- function(input, output) {
  ...
  observe({
    ...
    if(input$educ_level=="primaria"){
      df_reactive$df <- readRDS("../data/migration/agregado_dist_prim.rds")
    } else if (input$educ_level=="secundaria"){
      df_reactive$df <- readRDS("../data/migration/agregado_dist_sec.rds")
    }
    ...
    if(input$buff_size=="5k"){
      ...
      # calculate nodos
      df_reactive$nodos <- df_reactive$dfbuffers %>% 
        rename(grupo=buffer, name=cct, lat=latitud, lon=longitud)
      # calculate unions
      df_reactive$unions <- st_union(df_reactive$ch_df$ch_polygons) %>%  
        st_sf()
    } else if (input$buff_size=="10k"){
      df_reactive$dfbuffers <- readRDS("../data/buffers/df_buffers_10kms_sec.rds") %>% 
        rename(cct=name, buffer=buff_num) %>% 
        select(-X) %>% 
        select(cct, everything()) %>% 
        arrange(as.numeric(buffer))
      # buffer list
      df_reactive$buffers_list <- df_reactive$dfbuffers %>% unique() %>% 
        as.vector()
      # calculate convex hulls
      df_reactive$ch_df <- calc_convexhulls(df_reactive$dfbuffers)
      # calculate nodos
    ...
    need(try(values$selected_list <- comp_communities(
      buffer=values$cz_id, df_reactive$nodos, df_reactive$df, 
      values$selected_list$selected_nodos, 
