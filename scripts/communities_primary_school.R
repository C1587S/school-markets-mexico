...
"
This script computes school markets at primary school levels using 5 km and 10 km
buffers. 
...
#  Secundaria
df <- readRDS("../data/schools/agregado_dist_prim.rds")
# additionals
save <- TRUE
...
# Read buffer data
  if (type=="5kms"){
    dfbuffers <- readRDS("../data/buffers/df_buffers_5kms_prim.rds")$df_buffers  
  } else if(type=="10kms"){
    dfbuffers <- readRDS("../data/buffers/df_buffers_10kms_prim.rds")$df_buffers 
  }
  # buffer list
  buff_list <- dfbuffers %>% 
                  group_by(buffer) %>% 
  ...
  for (buff in buff_list){
    ...
    for (algo in algo_list){
      ...
      folder <- str_c("../data/results/school_markets/secundaria", type)
